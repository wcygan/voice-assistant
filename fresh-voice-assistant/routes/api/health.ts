  ...
  async GET(_req: Request, _ctx: FreshContext): Promise<Response> {
    const services = await Promise.all([
      ...
      checkService("whisper", async () => {
        const result =
          await $`../venv/bin/python -c "import whisper; print('OK')"`
            .noThrow();
        return result !== null;
      }),

      checkService("coqui_tts", async () => {
        const result = await $`../venv/bin/python -c "import TTS; print('OK')"`
          .noThrow();
        return result !== null;
      }),

      checkService("ollama", async () => {
        const result = await $`curl -s http://localhost:11434/api/tags`
          .noThrow();
        return result !== null;
      }),
