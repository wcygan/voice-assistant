...
async function processVoiceRequest(request: VoiceRequest): Promise<VoiceResponse> {
  ...
  log(`${YELLOW}📥 Received voice request: audio length=${audio.length}, model=${model}${RESET}`);
  ...
  try {
    ...
    log(`${CYAN}Command: ../venv/bin/whisper ${inputPath} --model ${whisperModel} --fp16 False --output_format txt --output_dir ${tempDir}${RESET}`);
    ...
    log(`${CYAN}Whisper result: ${transcriptResult ? 'SUCCESS' : 'FAILED'}${RESET}`);
...
`;
    ...
    log(`${CYAN}Encoding audio response (${audioResponse.length} bytes)${RESET}`);
...
export const handler = {
  ...
  async OPTIONS(_req: Request, _ctx: FreshContext): Promise<Response> {
