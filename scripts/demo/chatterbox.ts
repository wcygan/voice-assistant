...
console.log(
  `${BOLD}${MAGENTA}🎭 Chatterbox TTS Demo - Emotion-Controlled Speech${RESET}\n`,
);
console.log(
  "This demo showcases Chatterbox's unique ability to control emotional expression in synthesized speech.\n",
);
...
const emotionPresets: Record<
  string,
  { exaggeration: number; cfgWeight: number; description: string }
> = {
  "neutral": {
    exaggeration: 0.3,
    cfgWeight: 1.0,
  ...
  "sad": {
    exaggeration: 0.5,
    cfgWeight: 0.7,
    description: "Subdued and melancholic",
  },
  "calm": {
    exaggeration: 0.1,
    cfgWeight: 0.8,
  ...
  "serious": {
    exaggeration: 0.2,
    cfgWeight: 1.3,
    description: "Authoritative and firm",
  },
  "empathetic": {
    exaggeration: 0.4,
    cfgWeight: 0.9,
  ...
  "curious": {
    exaggeration: 0.6,
    cfgWeight: 1.1,
    description: "Inquisitive and thoughtful",
  },
};
...
async function checkDependencies(): Promise<boolean> {
  ...
  const torchCheck =
    await $`./venv/bin/python -c "import torch; print(f'CUDA: {torch.cuda.is_available()}')"`
      .noThrow();
  if (!torchCheck) {
  ...
  // Check Chatterbox
  const chatterboxCheck =
    await $`./venv/bin/python -c "from chatterbox.tts import ChatterboxTTS; print('OK')"`
      .noThrow();
  if (!chatterboxCheck) {
  ...
  console.log(
    `${GREEN}✅ Chatterbox TTS ready for emotion-controlled synthesis${RESET}\n`,
  );
  return true;
}
...
async function synthesizeWithEmotion(
  ...
  console.log(
    `${YELLOW}🎭 Synthesizing with ${emotion.toUpperCase()} emotion...${RESET}`,
  );
  console.log(
    `${BLUE}📝 Text: "${text.substring(0, 60)}${
      text.length > 60 ? "..." : ""
    }"${RESET}`,
  );
  console.log(
    `${BLUE}🎛️ Settings: exaggeration=${exaggeration}, cfg_weight=${cfgWeight}${RESET}`,
  );
  if (voiceRef) {
...
`;
  ...
  try {
    ...
    const result = await $`./venv/bin/python temp_chatterbox_demo.py 2>&1`
      .text();
...
async function runShowcase(outputDir: string): Promise<void> {
  ...
  const scenarios = [
    {
      ...
      text:
        "Hello! Welcome to our support center. I'm here to help you with any questions or concerns you might have today.",
      emotion: "happy",
    ...
    },
    {
      ...
      text:
        "Breaking news: A major scientific breakthrough has been announced. Researchers have made an incredible discovery that could change everything!",
      emotion: "excited",
    ...
    },
    {
      ...
      text:
        "I understand this must be really difficult for you. Please know that you're not alone, and I'm here to support you through this.",
      emotion: "empathetic",
    ...
    },
    {
      name: "Safety Warning",
      text:
        "Warning: Severe weather alert in your area. Please seek shelter immediately and stay away from windows. Your safety is our primary concern.",
      emotion: "serious",
    ...
    },
    {
      ...
      text:
        "Take a deep breath in... and slowly let it out. Allow your body to relax and your mind to find peace in this moment.",
      emotion: "calm",
    ...
    },
    {
      ...
      text:
        "That's a fascinating question! Let me think about this for a moment... I wonder if we could explore this from a different angle?",
      emotion: "curious",
    ...
    },
    {
      ...
      text:
        "I'm sorry to inform you that your request could not be processed at this time. I understand this may be disappointing.",
      emotion: "sad",
    ...
    },
    {
      ...
      text:
        "Your account balance is three hundred and forty-two dollars and fifty cents. Would you like to perform any other transactions?",
      emotion: "neutral",
  ...
  for (let i = 0; i < scenarios.length; i++) {
    ...
    console.log(
      `${YELLOW}🎬 Scenario ${
        i + 1
      }/${scenarios.length}: ${scenario.name}${RESET}`,
    );
  ...
  scenarios.forEach((s) => {
    console.log(
      `${CYAN}afplay "${outputDir}/${s.filename}" # ${s.name} (${s.emotion})${RESET}`,
    );
  });
}
...
async function runInteractiveMode(outputDir: string): Promise<void> {
  ...
  while (true) {
    ...
    // Get emotion
    console.log(
      `${YELLOW}Available emotions: ${
        Object.keys(emotionPresets).join(", ")
      }${RESET}`,
    );
    const emotion =
      prompt(`${CYAN}Choose emotion (default: neutral):${RESET} `) || "neutral";
    ...
    const outputPath = `${outputDir}/chatterbox_${
    ...
    if (result.success) {
      ...
      try {
        ...
        console.log(
          `${YELLOW}⚠️ Could not auto-play audio. To play manually:${RESET}`,
        );
...
async function runChatterboxDemo(
  ...
  if (!text) {
    console.log(
      `${YELLOW}📝 No text provided. Here are your options:${RESET}\n`,
    );
    console.log(
      `${CYAN}• Provide text: deno task demo:chatterbox --text "Your message" --emotion happy${RESET}`,
    );
    console.log(
      `${CYAN}• Interactive mode: deno task demo:chatterbox --interactive${RESET}`,
    );
    console.log(
      `${CYAN}• See showcase: deno task demo:chatterbox --showcase${RESET}`,
    );
    console.log(
      `${CYAN}• Voice cloning: deno task demo:chatterbox --text "Hello" --voice reference.wav${RESET}`,
    );
    console.log(
      `${CYAN}• Get help: deno task demo:chatterbox --help${RESET}\n`,
    );
    ...
    for (const [name, settings] of Object.entries(emotionPresets)) {
      ...
      console.log(
        `  ${BLUE}exaggeration=${settings.exaggeration}, cfg_weight=${settings.cfgWeight}${RESET}`,
      );
    }
    return;
  }
  ...
  if (result.success) {
    ...
    console.log(
      `${BOLD}🎭 Emotion:${RESET} ${emotion} - ${
        emotionPresets[emotion]?.description || "Custom"
      }`,
    );
    ...
    console.log(
      `${CYAN}• Fine-tune parameters: --exaggeration 0.8 --cfg-weight 1.2${RESET}`,
    );
    console.log(`${CYAN}• Clone a voice: --voice your_voice.wav${RESET}`);
    console.log(
      `${CYAN}• See all examples: deno task demo:chatterbox --showcase${RESET}`,
    );
  }
}
